...
impl MutableEntryData {
    ...
    pub fn try_new(e: String) -> Result<Self, crate::error::Error> {
        Ok(Self::new(EntryType::try_new(e)?))
    }

    pub fn check_and_insert(&mut self, k: String, v: String) -> Result<(), crate::error::Error> {
...
impl EntryData for MutableEntryData {
    ...
    fn get_field<'r>(&'r self, field_name: &str) -> Option<FieldValueRef<'r>> {
        self.get(field_name).map(FieldValue::ref_inner)
    }
}
